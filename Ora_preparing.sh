...
if [ $TMEM -le 2048 ]; then
    ...
    $ECHO "Do you want to enable it? (Y/y/Enter to accept, other to continue): \c"
    read A_HPAGE
    [ x"$A_HPAGE" == "x" ] || [ x"$A_HPAGE" == "xY" ] || [ x$A_HPAGE == "xy" ] && {
        $ECHO "Please input the expected total SGA size (include the ASM instance) in MB:\c"
        read A_SGASIZE
...
if [ "$ETSWAP" == "Y" ]; then
    ...
    Linux)
        ...
        if [ $? -eq 0 ]; then
            ...
            swapon $SWAPDEV
            $ECHO "Do you want to continue to other steps or extend the swap now?(c/C to continue, others to quit) \c"
            read A_CONFEXTSWAP
...
if [ x"$1" != x -a -f "$1" ]; then
    ...
    $ECHO
    $ECHO "OK for you? (Enter or input Y/y to confirm, any other to reject): \c"
    read A_CONFMID
    ...
    while read IDTYPE IDVALUE GUID; do
        case $IDTYPE in
...
EOF
    ...
    while [ $CONFMID != "YES" ]; do
        ...
        $ECHO
        $ECHO "Do you want to create seperate groups for Oracle software installation? (Y|N): \c"
        read SPGRP
        if [ "$SPGRP" == "Y" -o "$SPGRP" == "y" ]; then
            ...
            $ECHO "The Oracle software installation group is ${OIGRP}, OK for you? (Enter to accept, or input a new group name): \c"
            read A_OIGRP
            if [ "x$A_OIGRP" != "x" ]; then
                OIGRP=$A_OIGRP
            fi
        fi
        $ECHO "The default Oracle database administration group is dba, OK for you? (Enter to accept, or input a new group name): \c"
        read A_DBAGRP
        ...
        $ECHO "Do you want to make the $DBAGRP as the database operation group? (Enter to accept, or input a new group name): \c"
        read A_OPERGRP
        ...
        $ECHO "The default Oracle database user is oracle, OK for you? (Enter to accept, or input a new user name): \c"
        read A_DBAID
        if [ "x$A_DBAID" != "x" ]; then
            DBAID=$A_DBAID
        else
        ...
        $ECHO "Do you want to install GI (Y|N)? \c"
        read INGI
        if [ "$INGI" == "Y" -o "$INGI" == "y" ]; then
            $ECHO "Do you want to create seperated groups for GI installation? (Y|N)? \c"
            read SPGI
            $ECHO "The default ASM administration group is asmadmin, OK for you? (Enter to accept, or input a new group name): \c"
            read A_ASMADMGRP
            ...
            if [ "$SPGI" == "Y" -o "$SPGI" == "y" ]; then
                $ECHO "The default ASM DBA group is asmdba, OK for you? (Enter to accept, or input a new group name): \c"
                read A_ASMDBAGRP
                ...
                $ECHO "Do you want to make the $ASMADMGRP as the ASM operation group? (Enter to accept, or input a new group name): \c"
                read A_ASMOPERGRP
            ...
            $ECHO "The default GI user is grid, OK for you? (Enter to accept, or input a new user name): \c"
            read A_GIID
            if [ "x$A_GIID" != "x" ]; then
                GIID=$A_GIID
            else
        ...
        $ECHO
        $ECHO "OK for you? (Enter or input Y/y to confirm): \c"
        read A_CONFMID
...
Linux)
    ...
    if [ -h /etc/sysconfig/selinux ]; then
        ...
        if echo $SETARGET | grep '^/' >/dev/null; then
            SEFILE=$SETARGET
        else
    ...
    if [ -d /etc/dhcp ]; then
        ...
        if ! grep -Fxq "$DHCOPTIONS" /etc/resolv.conf; then
